...
tmux send-keys -t FIX_DistributedSystem:0.0 "cd '$CURRENT_DIR' && ./build/Release/fix_engine/src/FIXEngine" Enter
tmux send-keys -t FIX_DistributedSystem:0.1 "cd '$CURRENT_DIR' && ./build/Release/matching_engine/src/MatchingEngineServer" Enter
tmux send-keys -t FIX_DistributedSystem:0.2 "cd '$CURRENT_DIR' && sleep 5 && echo 'Starting clients...' && rm -rf ./client_simulator/tmp && ./venv/bin/python3.12 client_simulator/RunClients.py" Enter
