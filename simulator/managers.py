...
class ClientManager:
    ...
    def createClientConfig(self, clientConfig: dict[str, Any]) -> str:
        ...
        configContent = textwrap.dedent(f"""[DEFAULT]
                                            FileStorePath={cwd}/MessageStore/store_{clientID}/
                                            FileLogPath={cwd}/Logs/log_{clientID}/
        ...
        os.makedirs(f"{cwd}/MessageStore/store_{clientID}", exist_ok=True)
        os.makedirs(f"{cwd}/Logs/log_{clientID}", exist_ok=True)

        clientConfigsDir = f"{cwd}/ClientConfigs"
